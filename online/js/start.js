...
(function() {
  'use strict';
  var Ajax, BaseFunctionality, Cities, DublinCommuter, ForecastManager, LocalStorage, Location, LuasManager, OffsiteMenu, app, _ref, _ref1,
  ...
  */
  ...
  ForecastManager = (function(_super) {
    ...
    ForecastManager.prototype.populate = function() {
      var isInitialized;
      this.log("[Forecast] populate");
      isInitialized = this.cacheLoad();
      if (isInitialized) {
        this.emitEvent(ForecastManager.FORECAST_SUCCESS, [this.forecast]);
      }
      return this.log("[Forecast] populate result: " + isInitialized);
    };

    ForecastManager.prototype.cacheLoad = function() {
      var data, validForecast;
      this.log("[Forecast] cacheLoad " + this.id);
      this.hasForecast = false;
      data = this.retrieve(this.id);
      if (data != null) {
    ...
    ForecastManager.prototype.forecastSuccess = function(transport) {
      this.log("[Forecast] forecastSuccess");
    ...
    ForecastManager.prototype.populateForecast = function(data) {
      ...
      this.forecast.days = {};
      return this.log("[Forecast] populateForecast result: " + this.forecast);
    };

    ForecastManager.prototype.hasValidForecast = function() {
      var currentDateKey, status;
      this.log("[Forecast] hasValidForecast");
      status = false;
    ...
    ForecastManager.prototype.cacheSave = function() {
      var data, saved;
      this.log("[Forecast] cacheSave " + this.id);
      saved = false;
      data = {
    ...
    ForecastManager.prototype.destroy = function() {
      this.log("[Forecast] destroy " + this.id);
      this.remove(this.id);
      this.hasForecast = false;
      return this.forecast = {};
    };
    ...
    ForecastManager.prototype.celsius = function(f) {
      var c;
      this.log("[Forecast] celsius from: " + f);
      c = null;
      if (f != null) {
        c = (f - 32) * 5 / 9;
        c = Math.round(c);
  ...
  */
  ...
  DublinCommuter = (function(_super) {
    ...
    DublinCommuter.prototype.initialize = function() {
      ...
      this.weatherManager.addListener(ForecastManager.FORECAST_FAILED, this.handleWeatherForecastFailed);
      return this.offsiteMenu = new OffsiteMenu('#one-container-to-rule-them-all');
    };
